...
class Area():
    def __init__(self, line:str):
        rx = re.compile(f"(?P<mode>[a-z]*) x=(?P<x1>[\-0-9]*)..(?P<x2>[\-0-9]*),y=(?P<y1>[\-0-9]*)..(?P<y2>[\-0-9]*),z=(?P<z1>[\-0-9]*)..(?P<z2>[\-0-9]*)")
        match = rx.search(line)
        if match:
            self.mode = match["mode"]
            self.x1 = int(match["x1"])
            self.x2 = int(match["x2"])
            self.y1 = int(match["y1"])
            self.y2 = int(match["y2"])
            self.z1 = int(match["z1"])
            self.z2 = int(match["z2"])
    ...
    def Overlaps(self, otherarea) -> bool:
        return any([
            ...
            self.IsInside(otherarea.x1, otherarea.y2, otherarea.z1),
            self.IsInside(otherarea.x2, otherarea.y2, otherarea.z1),
            self.IsInside(otherarea.x1, otherarea.y1, otherarea.z2),
            self.IsInside(otherarea.x2, otherarea.y1, otherarea.z2),
            self.IsInside(otherarea.x1, otherarea.y2, otherarea.z2),
            self.IsInside(otherarea.x2, otherarea.y2, otherarea.z2)
        ])

    def Contains(self, otherarea) -> bool:
        return all([
            self.IsInside(otherarea.x1, otherarea.y1, otherarea.z1),
            self.IsInside(otherarea.x2, otherarea.y1, otherarea.z1),
            self.IsInside(otherarea.x1, otherarea.y2, otherarea.z1),
...
class Day22Solution(Aoc):
    ...
    def Run(self):
        self.StartDay(22, "Reactor Reboot")
        self.ReadInput()
        self.PartA()
        self.PartB()
    ...
    def PartA(self):
        ...
        reactor = [[[0 for _ in range(2 * size + 1)] for _ in range(2 * size + 1)] for _ in range(2 * size + 1)]
        for step in steps:
            if step.x1 < -size or step.x1 > size:
                continue
            print(step)
            self.DoStep(reactor, step, size)

        som = sum([sum([sum(y) for y in z]) for z in reactor])
        answer = som
